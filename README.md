...
Sunny-Canvas is a simple HTML document containing the JavaScript code that generates within an HTML canvas a sun moving at real time accross a generated sky that changes colour according to the time of day. Currently the default is set to demo time, and so the sun will move at a higher speed. In order to enable the real time movement change `var time = getDemoTime();` to `var time = getRealTime();` on line 74.
